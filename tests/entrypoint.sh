...
test_config() {
    ...
    for opt in "shutdown_correctly" "dns_type" "connection_type" "proxy_config_type" "fail_closed"; do
...
# ─────────────────────────────────────────────────────────────────
# Test: fail-closed protection (per-section opt-in)
# ─────────────────────────────────────────────────────────────────
test_fail_closed() {
    header "Fail-Closed Protection"
    ...
    if [ ! -r "$bin" ] || [ ! -r "$constants_lib" ] || [ ! -r "$cm_lib" ]; then
        skip "fail_closed (bin / constants / config manager not found)"
        return
    fi
    ...
    if [ -n "$SB_FAIL_CLOSED_REJECT_RULE_TAG" ]; then
        pass "fail_closed:constant — SB_FAIL_CLOSED_REJECT_RULE_TAG defined"
    else
        fail "fail_closed:constant — SB_FAIL_CLOSED_REJECT_RULE_TAG missing"
        return
    fi

  mkdir -p /usr/lib/netshift
  ln -sf "$jq_helpers" /usr/lib/netshift/helpers.jq 2>/dev/null || true
  # shellcheck disable=SC1090
  . "$cm_lib"
  ...
  cfg=$(sing_box_cm_add_reject_route_rule "$cfg" "$SB_FAIL_CLOSED_REJECT_RULE_TAG" "$SB_TPROXY_INBOUND_TAG")

  if echo "$cfg" | jq -e --arg tag "$SB_FAIL_CLOSED_REJECT_RULE_TAG" \
      '[.route.rules[] | select(.__service_tag == $tag and .action == "reject" and .inbound == "tproxy-in")] | length == 1' \
      > /dev/null 2>&1; then
      pass "fail_closed:route — catch-all reject rule shape OK"
  else
      fail "fail_closed:route — catch-all reject rule missing" "$(echo "$cfg" | jq -c '.route.rules' 2>/dev/null)"
  fi

  if command -v sing-box > /dev/null 2>&1; then
      local tmp="/tmp/netshift-fail-closed-$$.json"
      sing_box_cm_save_config_to_file "$cfg" "$tmp"
      if sing-box -c "$tmp" check > /dev/null 2>&1; then
          pass "fail_closed:singboxcheck — reject catch-all config validates"
      else
          fail "fail_closed:singboxcheck — sing-box rejected config" "$(sing-box -c "$tmp" check 2>&1)"
      fi
      rm -f "$tmp"
  else
      skip "fail_closed:singboxcheck — sing-box not installed"
  fi

  local helper_tmp="/tmp/netshift-fail-closed-helpers-$$.sh"
  cat > "$helper_tmp" << 'HLP'
set -e
BIN="BIN_PATH_PLACEHOLDER"
eval "$(awk '/^section_fail_closed_enabled\(\) \{/{p=1} p{print} p&&/^\}/{exit}' "$BIN")"
eval "$(awk '/^_check_fail_closed_section\(\) \{/{p=1} p{print} p&&/^\}/{exit}' "$BIN")"
eval "$(awk '/^netshift_fail_closed_active\(\) \{/{p=1} p{print} p&&/^\}/{exit}' "$BIN")"

config_get_bool() {
  eval "$1=\"\${4:-0}\""
  case "$3" in
    fail_closed) eval "$1=\"${FC_FAIL_CLOSED:-0}\"" ;;
  esac
}
config_get() {
  eval "$1=\"\${4:-}\""
  case "$3" in
    connection_type) eval "$1=\"${FC_CONN_TYPE:-proxy}\"" ;;
  esac
}
config_foreach() {
  [ "${FC_HAS_SECTION:-0}" = "1" ] && "$1" "main"
}

FC_HAS_SECTION=1
FC_CONN_TYPE=proxy
FC_FAIL_CLOSED=1
netshift_fail_closed_active && echo 'fc-active-on:OK' || echo 'fc-active-on:FAIL'

FC_FAIL_CLOSED=0
netshift_fail_closed_active && echo 'fc-active-off:FAIL' || echo 'fc-active-off:OK'

FC_FAIL_CLOSED=1
FC_CONN_TYPE=block
netshift_fail_closed_active && echo 'fc-block-type:FAIL' || echo 'fc-block-type:OK'
HLP
  sed -i "s|BIN_PATH_PLACEHOLDER|$bin|" "$helper_tmp"
  sh "$helper_tmp" 2>&1 | while IFS= read -r line; do
      case "$line" in
          *:OK) pass "fail_closed:helpers — $line" ;;
          *:FAIL) fail "fail_closed:helpers — $line" ;;
      esac
  done
  rm -f "$helper_tmp"
}
...
main() {
    ...
    case "$target" in
        ...
        *)
            ...
            echo "Available: all deps syntax config helpers jq cm sb nft nftv6 selmark isolation monfd unsupported textlist diagnostics subscription fastest insecure rejected jobstate selfheal dnsdetour suburlopt globalproxy failclosed stablecheck extcheck netshiftcheck latesttag ghredirect selfupdate backupguard"
            exit 1
            ;;
    esac
